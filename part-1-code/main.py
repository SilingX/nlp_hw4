...
def create_augmented_dataloader(args, dataset):
    ...
    n_aug = min(5000, len(train_raw))
    aug_sample = train_raw.shuffle(seed=42).select(range(n_aug))

    aug_transformed = aug_sample.map(custom_transform, load_from_cache_file=False)

    tokenized_orig = train_raw.map(tokenize_function, batched=True)
    tokenized_aug = aug_transformed.map(tokenize_function, batched=True, load_from_cache_file=False)
    ...
    tokenized_orig = prep(tokenized_orig)
    tokenized_aug = prep(tokenized_aug)

    from datasets import concatenate_datasets
    combined_train = concatenate_datasets([tokenized_orig, tokenized_aug])

    train_dataloader = DataLoader(combined_train, shuffle=True, batch_size=args.batch_size)
